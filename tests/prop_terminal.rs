//! Property tests for chunk-independent terminal feeding and parser
//! continuation equivalence.
//!
//! The oracle is a metamorphic relation: the same logical byte stream fed
//! through different feed partitions must leave observationally equivalent
//! terminal states, both right after a prefix and after every chunk of a
//! common suffix. The only guaranteed partition difference lies strictly
//! inside a completed target sequence; the prefix/suffix boundary itself is
//! never the differing cut.
//!
//! Reproduction:
